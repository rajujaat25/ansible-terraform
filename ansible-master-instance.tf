...
provisioner "remote-exec" {
    inline = [
      ...
      "sudo echo 'webserver1-master ansible_host=${self.public_ip} ansible_ssh_pass=ansible@123  ansible_user=ubuntu' >> /etc/ansible/hosts",
      "sudo echo 'webserver2-worker ansible_host=${aws_instance.ansible-worker-instance.public_ip} ansible_ssh_pass=worker@123  ansible_user=ubuntu' >> /etc/ansible/hosts",
